...
class HathorWalletServiceWallet extends EventEmitter implements IHathorWallet {
  ...
  async prepareMintTokensData(
    ...
    const mappedOutputs = outputsObj.map(output => {
      let authorities = 0n;
    ...
    const fee = await Fee.calculate(this.storage, [], mappedOutputs);
  ...
  async prepareMeltTokensData(
    ...
    if (tokenData?.version === TokenInfoVersion.FEE) {
      const fee = await Fee.calculate(this.storage, inputsObj, outputsObj);
