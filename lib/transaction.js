...
const transaction = {
  ...
  validateAddress(address, addressBytes) {
    const errorMessage = `Invalid address: ${address}`; // Validate address length
  ...
  createOutputScript(address, timelock) {
    ...
    if (this.validateAddress(address, addressBytes)) {
      ...
      arr.push(OP_DUP);
      arr.push(OP_HASH160); // addressHash has a fixed size of 20 bytes, so no need to push OP_PUSHDATA1
  ...
  dataToSign(txData) {
    ...
    arr.push(this.intToBytes(DEFAULT_TX_VERSION, 2)); // Len inputs

    arr.push(this.intToBytes(txData.inputs.length, 1)); // Len outputs

    arr.push(this.intToBytes(txData.outputs.length, 1)); // Len tokens
    ...
    for (let inputTx of txData.inputs) {
      ...
      arr.push(this.intToBytes(inputTx.index, 1)); // Input data will be fixed to 0 for now

      arr.push(this.intToBytes(0, 2));
    }

    for (let outputTx of txData.outputs) {
      arr.push(this.outputValueToBytes(outputTx.value)); // Token data
  ...
  calculateTxWeight(txData) {
    let txSize = this.txToBytes(txData).length; // XXX Parents are calculated only in the server but we need to consider them here
    ...
    let weight = txWeightConstants.txWeightCoefficient * Math.log2(txSize) + 4 / (1 + txWeightConstants.txMinWeightK / amount) + 4; // Make sure the calculated weight is at least the minimum

    weight = Math.max(weight, txWeightConstants.txMinWeight); // FIXME precision difference between backend and frontend (weight (17.76246721531992) is smaller than the minimum weight (17.762467215319923))
   ...
   */
  txToBytes(txData) {
    let arr = []; // Serialize first the funds part
    //
    // Tx version

    arr.push(this.intToBytes(DEFAULT_TX_VERSION, 2)); // Len tokens

    arr.push(this.intToBytes(txData.tokens.length, 1)); // Len inputs
    ...
    arr.push(this.floatToBytes(txData.weight, 8)); // Timestamp

    arr.push(this.intToBytes(txData.timestamp, 4)); // Len parents (parents will be calculated in the backend)

    arr.push(this.intToBytes(0, 1)); // Add nonce in the end
  ...
  sendTransaction(data, pin) {
    ...
    data = transaction.signTx(data, dataToSign, pin); // Completing data in the same object
  ...
  getTransactionWeightConstants() {
    return {
      'txMinWeight': parseFloat(localStorage.getItem('wallet:txMinWeight')),
