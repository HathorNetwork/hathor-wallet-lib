...
class WS extends EventEmitter {
  constructor() {
    if (!WS.instance) {
      super(); // Boolean to show when there is a websocket started with the server

      this.started = false; // Boolean to show when the websocket connection is working

      this.connected = undefined; // Store variable that is passed to Redux if ws is online
  ...
  setIsOnline(value) {
    ...
    if (this.isOnline !== value) {
      this.isOnline = value; // Emits event of online state change
