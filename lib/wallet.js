...
var wallet = {
  ...
  executeGenerateWallet: function executeGenerateWallet(words, passphrase, pin, password, loadHistory) {
    ...
    if (loadHistory) {
      ...
      this.loadAddressHistory(0, _constants.GAP_LIMIT);
    }
    return code.phrase;
  },
  ...
  isPinCorrect: function isPinCorrect(pin) {
    var data = JSON.parse(localStorage.getItem('wallet:accessData'));
   ...
   * @inner
   */
  hasTokenAndAddress: function hasTokenAndAddress(tx, selectedToken) {
  ...
  reloadData: function reloadData() {
    // Get old access data
    var accessData = JSON.parse(localStorage.getItem('wallet:accessData'));
    var walletData = this.getWalletData();
    ...
    var dataToken = _tokens2.default.getTokens();
    ...
    return this.loadAddressHistory(0, _constants.GAP_LIMIT);
  },
   ...
   * @inner
   */
  updateHistoryData: function updateHistoryData(oldHistoryTransactions, oldAllTokens, newHistory, resolve) {
    var dataJson = this.getWalletData();
    ...
    return { historyTransactions: historyTransactions, allTokens: allTokens, newSharedAddress: newSharedAddress, newSharedIndex: newSharedIndex };
  },
   ...
   * @inner
   */
  isAddressMine: function isAddressMine(address) {
    var data = this.getWalletData();
