...
const wallet = {
  ...
  wordsValid(words) {
    if (_.isString(words)) {
      if (words.split(' ').length !== 24) {
        ...
        return {
          'valid': false,
          'message': 'Must have 24 words'
        };
      } else if (!Mnemonic.isValid(words)) {
        // Invalid sequence of words
        return {
          'valid': false,
          'message': 'Invalid sequence of words'
        };
      }
    } else {
      // Must be string
  ...
  executeGenerateWallet(words, passphrase, pin, password, loadHistory) {
    ...
    let encryptedDataWords = this.encryptData(words, password); // Save in localStorage the encrypted private key and the hash of the pin and password
  ...
  loadAddressHistory(startIndex, count) {
    const promise = new Promise((resolve, reject) => {
      ...
      for (var i = startIndex; i < stopIndex; i++) {
        ...
        dataJson.keys[address.toString()] = {
          privkey: null,
          index: i
        };
        addresses.push(address.toString()); // Subscribe in websocket to this address updates
      ...
      walletApi.getAddressHistory(addresses, response => {
        const data = this.getWalletData(); // Update historyTransactions with new one
  ...
  generateNewAddress() {
    ...
    const xpub = HDPublicKey(dataJson.xpubkey); // Get last shared index to discover new index

    const lastSharedIndex = this.getLastSharedIndex();
    let newIndex = lastSharedIndex + 1;
    const newKey = xpub.derive(newIndex);
    const newAddress = Address(newKey.publicKey, NETWORK); // Update address data and last generated indexes
    ...
    data.keys[newAddress.toString()] = {
      privkey: null,
      index: newIndex
    };
    data = localStorage.memory ? data : JSON.stringify(data);
    localStorage.setItem('wallet:data', data); // Subscribe in ws to new address updates
   ...
   */
  getAddressToUse() {
    const address = localStorage.getItem('wallet:address'); // Updating address because the last one was used
  ...
  checkUnspentTxExists(historyTransactions, txId, index, selectedToken) {
    ...
    for (const tx_id in historyTransactions) {
      ...
      if (tx.is_voided) {
        ...
        return {
          exists: false,
          message: `Transaction [${txId}] is voided`
        };
      }
      ...
      if (tx.outputs.length - 1 < index) {
        ...
        return {
          exists: false,
          message: `Transaction [${txId}] does not have this output [index=${index}]`
        };
      }
      ...
      if (this.isAuthorityOutput(txout)) {
        ...
        return {
          exists: false,
          message: `Output [${index}] of transaction [${txId}] is an authority output`
        };
      }
      ...
      if (!this.isAddressMine(txout.decoded.address, data)) {
        return {
          exists: false,
          message: `Output [${index}] of transaction [${txId}] is not yours`
        };
      }
      ...
      if (txout.token !== selectedToken) {
        return {
          exists: false,
          message: `Output [${index}] of transaction [${txId}] is not from selected token [${selectedToken}]`
        };
      }
      ...
      if (txout.spent_by !== null) {
        return {
          exists: false,
          message: `Output [${index}] of transaction [${txId}] is already spent`
        };
      }
    ...
    return {
      exists: false,
      message: `Transaction [${txId}] does not exist in the wallet`
    };
  },
  ...
  reloadData() {
    ...
    const walletData = this.getWalletData();
    this.cleanWallet(); // Restart websocket connection

    WebSocketHandler.setup();
    let newWalletData = {
      keys: {},
      xpubkey: walletData.xpubkey // Prepare to save new data
    ...
    localStorage.setItem('wallet:data', newWalletData); // Load history from new server
  ...
  prepareSendTokensData(data, token, chooseInputs, historyTransactions, allTokens) {
    ...
    if (outputsAmount === 0) {
      return {
        success: false,
        message: `Token: ${token.symbol}. Total value can't be 0`
      };
    }
    ...
    if (chooseInputs) {
      ...
      if (newData.inputsAmount > outputsAmount) {
        ...
        data['outputs'].push(outputChange); // Shuffle outputs, so we don't have change output always in the same index
      ...
      for (const input of data.inputs) {
        ...
        if (!utxo.exists) {
          return {
            success: false,
            message: `Token: ${token.symbol}. ${utxo.message}`
          };
        }
        ...
        if (this.canUseUnspentTx(output)) {
          ...
          return {
            success: false,
            message: `Token: ${token.symbol}. Output [${input.tx_id}, ${input.index}] is locked until ${dateFormatter.parseTimestamp(output.decoded.timelock)}`
          };
        }
      }
      ...
      if (inputsAmount < outputsAmount) {
        return {
          success: false,
          message: `Token: ${token.symbol}. Sum of outputs is larger than the sum of inputs`
        };
      }
  ...
  updateHistoryData(oldHistoryTransactions, oldAllTokens, newHistory, resolve, dataJson) {
    ...
    if (maxIndex > lastUsedIndex && lastUsedAddress !== null) {
      ...
      this.setLastUsedIndex(lastUsedAddress); // Setting last shared address, if necessary
    ...
    const lastGeneratedIndex = this.getLastGeneratedIndex(); // Just in the case where there is no element in all data
