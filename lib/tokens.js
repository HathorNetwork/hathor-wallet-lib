...
const tokens = {
  ...
  validateTokenToAddByConfigurationString(config, uid) {
    ...
    if (uid && uid !== tokenData.uid) {
      return {
        success: false,
        message: `Configuration string uid does not match: ${uid} != ${tokenData.uid}`
      };
    }
  ...
  validateTokenToAddByUid(uid) {
    ...
    if (existedToken) {
      return {
        success: false,
        message: `You already have this token: ${uid} (${existedToken.name})`
      };
    }
  ...
  getTokenFromConfigurationString(config) {
    ...
    const symbol = configArr.pop(); // Assuming that the name might have : on it
  ...
  createToken(input, output, address, name, symbol, mintAmount, pin) {
    ...
    const tokenMasks = TOKEN_CREATION_MASK | TOKEN_MINT_MASK | TOKEN_MELT_MASK; // Authority output token data

    const tokenData = 0b10000001; // Create token uid
    ...
    let txData = {
      'inputs': [input],
      'outputs': [authorityOutput, output],
      'tokens': [tokenUID]
    }; // Get data to sign

    const dataToSign = transaction.dataToSign(txData); // Sign tx

    txData = transaction.signTx(txData, dataToSign, pin); // Assemble tx and send to backend
  ...
  mintTokens(txId, token, address, amount, pin) {
    const promise = new Promise((resolve, reject) => {
      ...
      const tokenData = 0b10000001; // Now we will mint the tokens
      ...
      let newTxData = {
        'inputs': [newInput],
        'outputs': [tokenOutput1, tokenOutput2, tokenOutput3],
        'tokens': [token]
      };

      try {
        // Get new data to sign
        const newDataToSign = transaction.dataToSign(newTxData); // Sign mint tx

        newTxData = transaction.signTx(newTxData, newDataToSign, pin); // Assemble tx and send to backend
