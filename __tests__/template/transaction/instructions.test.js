...
describe('parsing variable references', () => {
  ...
  it('should get a template variable from reference', () => {
    ...
    expect(getVariable('{foo2}', { foo2: 'bar' }, z.string().transform(o => `${o}1`))).toBe('bar1');
    expect(getVariable(`{foo3}`, { foo3: 10 }, z.coerce.bigint())).toBe(10n);
    // Validation/refinement should be applied to value
    expect(getVariable('baz', {}, z.string().transform(o => `${o}1`))).toBe('baz1');
    expect(getVariable(11, {}, z.coerce.bigint())).toBe(11n);
    // Should work with any schema
    expect(getVariable('{objfoo}', { objfoo: { foo: '10' } }, z.object({ foo: z.coerce.bigint() }))).toStrictEqual({ foo: 10n });
...
describe('should parse template instructions', () => {
  ...
  it('should parse RawInputInstruction', () => {
    expect(RawInputInstruction.safeParse({
      type: 'input/raw',
      position: 0,
      index: 1,
      txId,
    ...
    expect(RawInputInstruction.parse({
      type: 'input/raw',
      index: '10',
      txId: '{foo}',
      foo: 'bar',
  ...
  it('should parse UtxoSelectInstruction', () => {
    expect(UtxoSelectInstruction.safeParse({
      type: 'input/utxo',
      position: 0,
      fill: 10,
      token,
    ...
    // Parse with default
    expect(UtxoSelectInstruction.parse({
      type: 'input/utxo',
      fill: 10,
    })).toStrictEqual({
      type: 'input/utxo',
  ...
  it('should parse AuthoritySelectInstruction', () => {
    expect(AuthoritySelectInstruction.safeParse({
    ...
    // Parse with defaults
    expect(AuthoritySelectInstruction.parse({
      type: 'input/authority',
      authority: 'melt',
      token,
    })).toStrictEqual({
    ...
    expect(AuthoritySelectInstruction.safeParse({ // Missing authority
      type: 'input/authority',
      token,
    }).success).toBe(false);
    expect(AuthoritySelectInstruction.safeParse({ // Missing token
      type: 'input/authority',
      authority: 'melt',
    }).success).toBe(false);
    expect(AuthoritySelectInstruction.safeParse({
  ...
  it('should parse RawOutputInstruction', () => {
    expect(RawOutputInstruction.safeParse({
      type: 'output/raw',
      position: 0,
      amount: '10',
      script: 'cafe',
    ...
    // Parse with defaults
    expect(RawOutputInstruction.parse({
      type: 'output/raw',
      script: 'cafe',
    })).toStrictEqual({
      type: 'output/raw',
    ...
    expect(RawOutputInstruction.safeParse({ // Missing script
      type: 'output/raw',
    }).success).toBe(false);
    expect(RawOutputInstruction.safeParse({
      type: 'output/raw',
  ...
  it('should parse TokenOutputInstruction', () => {
    expect(TokenOutputInstruction.safeParse({
      type: 'output/token',
      position: 0,
      amount: '10',
      token,
    ...
    // Parse with defaults
    expect(TokenOutputInstruction.parse({
      type: 'output/token',
      amount: '5',
      address,
    })).toStrictEqual({
    ...
    expect(TokenOutputInstruction.safeParse({ // Missing amount
      type: 'output/token',
      address,
    }).success).toBe(false);
    expect(TokenOutputInstruction.safeParse({ // Missing address
      type: 'output/token',
      amount: 10,
    }).success).toBe(false);
  });
  ...
  it('should parse AuthorityOutputInstruction', () => {
    expect(AuthorityOutputInstruction.safeParse({
    ...
    // Parse with defaults
    expect(AuthorityOutputInstruction.parse({
      type: 'output/authority',
      token,
      authority: 'mint',
      address,
    ...
    expect(AuthorityOutputInstruction.safeParse({ // Missing token
      type: 'output/authority',
      authority: 'melt',
      address,
    }).success).toBe(false);
    ...
    expect(AuthorityOutputInstruction.safeParse({ // Missing authority
      type: 'output/authority',
      token,
      address,
    }).success).toBe(false);
    expect(AuthorityOutputInstruction.safeParse({ // Missing address
      type: 'output/authority',
      token,
      authority: 'mint',
    }).success).toBe(false);
  ...
  it('should parse DataOutputInstruction', () => {
    expect(DataOutputInstruction.safeParse({
      type: 'output/data',
      position: 0,
      data: 'foo',
      token,
    ...
    // Parse with defaults
    expect(DataOutputInstruction.parse({
      type: 'output/data',
      data: 'foo',
    })).toStrictEqual({
      type: 'output/data',
    ...
    expect(DataOutputInstruction.safeParse({ // Missing data
      type: 'output/data',
    }).success).toBe(false);
  });

  it('should parse ShuffleInstruction', () => {
    expect(ShuffleInstruction.safeParse({
      type: 'action/shuffle',
      target: 'all',
    }).success).toBe(true);
    expect(ShuffleInstruction.safeParse({
    ...
    expect(ShuffleInstruction.safeParse({ // missing target
      type: 'action/shuffle',
    }).success).toBe(false);
    expect(ShuffleInstruction.safeParse({
  ...
  it('should parse ChangeInstruction', () => {
    expect(ChangeInstruction.safeParse({
      type: 'action/change',
      token,
      address,
      timelock: 456,
    ...
    // Parse with defaults
    expect(ChangeInstruction.parse({
      type: 'action/change',
    })).toStrictEqual({
      type: 'action/change',
    });
  ...
  it('should parse CompleteInstruction', () => {
    expect(CompleteTxInstruction.safeParse({
    ...
    // Parse with defaults
    expect(CompleteTxInstruction.parse({
      type: 'action/complete',
    })).toStrictEqual({
      type: 'action/complete',
    });
  ...
  it('should parse ConfigInstruction', () => {
    ...
    // Parse with defaults
    expect(ConfigInstruction.parse({
      type: 'action/config',
    })).toStrictEqual({
      type: 'action/config',
    });
  ...
  it('should parse SetVarInstruction', () => {
    expect(SetVarInstruction.safeParse({
      type: 'action/setvar',
      name: 'foo',
      value: 'anything',
    }).success).toBe(true);
...
describe('Template schemes', () => {
  ...
  it('should parse any template instruction', () => {
    expect(TxTemplateInstruction.safeParse({
      type: 'input/raw',
      index: 1,
      txId,
    }).success).toBe(true);
  ...
  it('should parse a simple template', () => {
    expect(TransactionTemplate.safeParse([
      {
        type: 'action/config',
        version: 3,
        tokenName: 'foo',
