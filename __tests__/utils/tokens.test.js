...
test('Validate configuration string', async () => {
  ...
  await expect(tokens.validateTokenToAddByConfigurationString('invalid-string')).rejects.toThrow('Invalid configuration string');
  await expect(tokens.validateTokenToAddByConfigurationString('invalid-string')).rejects.toThrow(TokenValidationError);
  await expect(tokens.validateTokenToAddByConfigurationString('invalid-string', storage)).rejects.toThrow(TokenValidationError);

  // Should throw if uid does not match the expected uid
  await expect(tokens.validateTokenToAddByConfigurationString(configString, null, 'expected-uid')).rejects.toThrow('Configuration string uid does not match');
  await expect(tokens.validateTokenToAddByConfigurationString(configString, null, 'expected-uid')).rejects.toThrow(TokenValidationError);
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage, 'expected-uid')).rejects.toThrow(TokenValidationError);

  // should throw if token is already registered
  await store.registerToken({ uid, name, symbol });
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow('You already have this token');
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow(TokenValidationError);
  ...
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow('You already have a token with this name');
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow(TokenValidationError);
  await store.unregisterToken(uid2);

  await store.registerToken({ uid: uid2, name: 'Another test name', symbol: 'TST' });
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow('You already have a token with this symbol');
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow(TokenValidationError);
  await store.unregisterToken(uid2);
  ...
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow('boom!');
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow(TokenValidationError);

  apiSpy.mockImplementation((_, cb) => {
    cb({ success: true, name: 'Another name', symbol });
  });
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow('Token name does not match');
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow(TokenValidationError);

  apiSpy.mockImplementation((_, cb) => {
    cb({ success: true, name, symbol: 'Another symbol' });
  });
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow('Token symbol does not match');
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).rejects.toThrow(TokenValidationError);
  ...
  await expect(tokens.validateTokenToAddByConfigurationString(configString, storage)).resolves.toEqual({ uid, name, symbol});
