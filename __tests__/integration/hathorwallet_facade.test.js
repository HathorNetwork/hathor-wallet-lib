...
describe('getTxById', () => {
  ...
  it('should return tx token balance', async () => {
    ...
    const tx1 = await GenesisWalletHelper.injectFunds(hWallet, fundDestinationAddress, 10);
...
describe('start', () => {
  ...
  it('should start a wallet to manage a specific token', async () => {
    ...
    await GenesisWalletHelper.injectFunds(hWallet, await hWallet.getAddressAtIndex(0), 2);
    const { hash: tokenUid } = await createTokenHelper(
      hWallet,
  ...
  it('should start a wallet via xpub', async () => {
    ...
    await expect(hWallet.getBalance(HATHOR_TOKEN_UID)).resolves.toStrictEqual([
      expect.objectContaining({
        token: expect.objectContaining({ id: HATHOR_TOKEN_UID }),
        balance: { unlocked: 0, locked: 0 },
        transactions: 0,
      }),
    ]);
    ...
    await expect(hWallet.getBalance(HATHOR_TOKEN_UID)).resolves.toMatchObject([
      expect.objectContaining({
        token: expect.objectContaining({ id: HATHOR_TOKEN_UID }),
...
describe('addresses methods', () => {
  ...
  it('should get the correct current/next addresses', async () => {
    ...
    currentAddress = await hWallet.getCurrentAddress();
    await GenesisWalletHelper.injectFunds(hWallet, currentAddress.address, 1);
...
describe('getBalance', () => {
  ...
  it('should get the balance for the HTR token', async () => {
    ...
    const balance = await hWallet.getBalance(HATHOR_TOKEN_UID);
    expect(balance).toHaveLength(1);
    expect(balance[0]).toMatchObject({
      token: { id: HATHOR_TOKEN_UID },
      balance: { unlocked: 0, locked: 0 },
      transactions: 0,
    });
    ...
    const balance1 = await hWallet.getBalance(HATHOR_TOKEN_UID);
    ...
    const balance2 = await hWallet.getBalance(HATHOR_TOKEN_UID);
...
describe('getTxBalance', () => {
  ...
  it('should get tx balance', async () => {
    ...
    expect(txBalance[HATHOR_TOKEN_UID]).toEqual(0);
    expect(txBalance).toHaveProperty(tokenUid, 0);
  });
});
...
describe('sendTransaction', () => {
  ...
  it('should send HTR transactions', async () => {
    ...
    let htrBalance = await hWallet.getBalance(HATHOR_TOKEN_UID);
    ...
    // Balance was reduced
    htrBalance = await hWallet.getBalance(HATHOR_TOKEN_UID);
  ...
  it('should send a multisig transaction', async () => {
    ...
    await GenesisWalletHelper.injectFunds(mhWallet1, await mhWallet1.getAddressAtIndex(0), 10);
...
describe('mintTokens', () => {
  ...
  it('should deposit correct HTR values for minting', async () => {
    ...
    async function getHtrBalance(hWallet) {
      const [htrBalance] = await hWallet.getBalance(HATHOR_TOKEN_UID);
...
describe('getToken methods', () => {
  ...
  it('should get the correct responses for a valid token', async () => {
    ...
    expect(getTokensResponse[0]).toEqual(HATHOR_TOKEN_UID);
    ...
    expect(getTokensResponse).toStrictEqual([HATHOR_TOKEN_UID, tokenUid]);
