...
import { HATHOR_TOKEN_UID, TOKEN_MELT_MASK, TOKEN_MINT_MASK } from '../../src/constants';
import {
...
describe('getAddressInfo', () => {
  ...
  it('should display correct values for HTR transactions with no change', async () => {
    ...
    await expect(hWallet.getAddressInfo(addr0)).resolves.toMatchObject({
      ...
      token: HATHOR_TOKEN_UID, // Validating this field only once to ensure it's correct
      index: 0,
    });
...
describe('getTxAddresses', () => {
  it('should identify transaction addresses correctly', async () => {
    ...
    const tx = await gWallet.sendManyOutputsTransaction(
      [
        { address: await hWallet.getAddressAtIndex(1), value: 1, token: HATHOR_TOKEN_UID },
        { address: await hWallet.getAddressAtIndex(3), value: 3, token: HATHOR_TOKEN_UID },
        { address: await hWallet.getAddressAtIndex(5), value: 5, token: HATHOR_TOKEN_UID },
      ],
      {
...
describe('getUtxosForAmount', () => {
  ...
  it('should filter by custom token', async () => {
    ...
    await expect(
      hWallet.getUtxosForAmount(6, { token: HATHOR_TOKEN_UID })
    ).resolves.toStrictEqual({
      changeAmount: expect.any(Number),
      utxos: [expect.objectContaining({ tokenId: HATHOR_TOKEN_UID })],
    });
...
describe('consolidateUtxos', () => {
  ...
  it('should throw when consolidating on an invalid address', async () => {
    const fundTx = await hWallet2.sendManyOutputsTransaction([
      { address: await hWallet1.getAddressAtIndex(0), value: 4, token: HATHOR_TOKEN_UID },
      { address: await hWallet1.getAddressAtIndex(1), value: 5, token: HATHOR_TOKEN_UID },
    ]);
    ...
    await cleanWallet1(HATHOR_TOKEN_UID);
  });
  ...
  it('should consolidate with filter_address filter', async () => {
    ...
    const fundTx = await hWallet2.sendManyOutputsTransaction([
      { address: addr1, value: 1, token: HATHOR_TOKEN_UID },
      { address: addr1, value: 2, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 1, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 1, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 3, token: HATHOR_TOKEN_UID },
    ]);
    await waitForTxReceived(hWallet1, fundTx.hash);
    await waitForTxReceived(hWallet2, fundTx.hash);
  ...
  it('should consolidate with amount_smaller_than filter', async () => {
    ...
    const fundTx = await hWallet2.sendManyOutputsTransaction([
      { address: addr1, value: 1, token: HATHOR_TOKEN_UID },
      { address: addr1, value: 2, token: HATHOR_TOKEN_UID },
      { address: addr1, value: 3, token: HATHOR_TOKEN_UID },
      { address: addr1, value: 4, token: HATHOR_TOKEN_UID },
      { address: addr1, value: 5, token: HATHOR_TOKEN_UID },
    ]);
    await waitForTxReceived(hWallet1, fundTx.hash);
    await waitForTxReceived(hWallet2, fundTx.hash);
  ...
  it('should consolidate with amount_bigger_than filter', async () => {
    ...
    const fundTx = await hWallet2.sendManyOutputsTransaction([
      { address: addr2, value: 1, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 2, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 3, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 4, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 5, token: HATHOR_TOKEN_UID },
    ]);
    await waitForTxReceived(hWallet1, fundTx.hash);
    await waitForTxReceived(hWallet2, fundTx.hash);
  ...
  it('should consolidate with amount_bigger_than and max_amount filter', async () => {
    ...
    const fundTx = await hWallet2.sendManyOutputsTransaction([
      ...
      { address: addr2, value: 4, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 5, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 20, token: HATHOR_TOKEN_UID },
      { address: addr2, value: 40, token: HATHOR_TOKEN_UID },
    ]);
    await waitForTxReceived(hWallet1, fundTx.hash);
    await waitForTxReceived(hWallet2, fundTx.hash);
