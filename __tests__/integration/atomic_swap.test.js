...
describe('partial tx proposal', () => {
  ...
  it('Should exchange tokens between wallets', async () => {
    ...
    const w1HTRBefore = await hWallet1.getBalance(HATHOR_TOKEN_UID);
    const w1Tk1Before = await hWallet1.getBalance(token1Uid);
    const w1Tk2Before = await hWallet1.getBalance(token2Uid);

    const w2HTRBefore = await hWallet2.getBalance(HATHOR_TOKEN_UID);
    ...
    await proposal1.addSend(HATHOR_TOKEN_UID, 100);
    ...
    await proposal2.addReceive(HATHOR_TOKEN_UID, 100);
    ...
    // Get the balance states before the exchange
    const w1HTRAfter = await hWallet1.getBalance(HATHOR_TOKEN_UID);
    const w1Tk1After = await hWallet1.getBalance(token1Uid);
    const w1Tk2After = await hWallet1.getBalance(token2Uid);

    const w2HTRAfter = await hWallet2.getBalance(HATHOR_TOKEN_UID);
